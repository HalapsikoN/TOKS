...
void COM::Write(uint16_t* data, int datasize)
{
	if (m_Handle == 0) { throw Exception(); }
	DWORD feedback;
	if (!WriteFile(m_Handle, data, datasize, &feedback, 0) ||
...
void COM::Read(uint16_t* data, int bufsize) {
	DWORD feedback = 0;
	int attempts = 5; //количество запросов на чтение
	while (bufsize && attempts) {
		if (attempts) attempts--;
...
void COM::WriteHeming(const uint8_t* data)
{
	int i;
	for (i = 0; *(data + i) != '\0'; ++i)
	{
...
void COM::ReadHeming(uint8_t* data) {
	int i;
	uint16_t temp = 'a';
	for (i = 0; temp != '\0'; ++i)
	{
