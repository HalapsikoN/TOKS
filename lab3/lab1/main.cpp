...
firstPackageWrite=0, firstPackageRead = 0, 
firstHemingWrite=0, firstHemingRead=0;
int secondWrite = 0, secondRead = 0, secondChangeBaydrate = 0, 
secondPackageWrite = 0, secondPackageRead = 0, 
secondHemingWrite = 0, secondHemingRead = 0;
...
void portCOM1()
{
	...
	while (work)
	{
		...
		if (firstHemingWrite)
		{
			cout << "write Heming COM1:" << endl;
			rewind(stdin);
			cin.getline(str, 200);
			firstPort.WriteHeming((uint8_t*)& str);
			firstHemingWrite = 0;
		}
		if (firstHemingRead)
		{
			cout << "read Heming COM1:" << endl;
			firstPort.ReadHeming((uint8_t*)& str);
			cout << str << endl;
			firstHemingRead = 0;
		}
...
void portCOM2()
{
	...
	while (work)
	{
		...
		if (secondHemingWrite)
		{
			cout << "write Heming COM2:" << endl;
			rewind(stdin);
			cin.getline(str, 200);
			secondPort.WriteHeming((uint8_t*)& str);
			secondHemingWrite = 0;
		}
		if (secondHemingRead)
		{
			cout << "read Heming COM2:" << endl;
			secondPort.ReadHeming((uint8_t*)& str);
			cout << str << endl;
			secondHemingRead = 0;
		}
		ReleaseSemaphore(hSemaphore, 1, NULL);
...
{	
	...
	while (work)
	{
		do
		{
			...
			cout << "7) write from first to second (HEMING)" << endl;
			cout << "8) write from second to first (HEMING)" << endl;
			cout << "0) exit" << endl << endl;
		} while (!(scanf_s("%d", &choice)) || choice > 8 || choice < 0);
